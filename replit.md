...
- **B.3.2.2 — State machine timeouts** ✅ CODE READY (VPS test pending) — `who_proposes` extended with `round` parameter (`(h+r) % len`); `Producer::run()` rewritten as state machine with `PROPOSE_TIMEOUT_SECS=8s` and `TICK_INTERVAL_MS=500ms`. Round 0 honours `BLOCK_TIME_SECS=5s` pacing; recovery rounds (≥1) propose immediately. If elected proposer at (h, r=0) doesn't get a block committed within 8s, every node bumps to r=1 and re-elects (other validator takes over). Tip-advance auto-resets to r=0. 4 unit tests pass. VPS test plan: kill one node → expect `⏰ propose timeout` log + the OTHER node taking over after 8s.
