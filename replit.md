...
## Multisig Wallet Tools — Full Upgrade (Create + Import)
The Multisig Explorer's "Wallet Tools" component (`artifacts/sui-fork-dashboard/src/components/multisig/WalletTools.tsx`) was rewritten from a 2-tab CLI planner into a 3-tab Create / Import / Watchlist experience that integrates the in-browser AES-GCM vault and chain RPC.

- **Create tab**: live keccak-derived address preview is unchanged, but now reads vault wallets via `loadWallets()` and exposes a chip-row picker that auto-fills the creator address (handling locked / empty / read-error vault states gracefully). A "Add Creator" button inserts the creator into the owner list with one click. After the user runs the generated CLI command on the host, a "Verify on-chain" action polls `zbx_getMultisig` every 4 s for up to 90 s and auto-bookmarks the resulting wallet with full metadata once the chain reports it.
- **Import tab (new)**: three modes — Single (paste → fetch → preview owners/threshold/balance/created-height with ownership detection → save), Bulk (textarea, one address per line → batch verify with per-row status badges → multi-select save), JSON (file upload or paste with merge / replace).
- **Watchlist tab**: each entry now caches `MultisigMetadata` (owners, threshold, balanceWei + balanceZbx, createdHeight, proposalSeq, checkedAt). On mount the panel auto-refreshes any entry stale > 5 min. Per-row refresh button, inline rename, ownership chip ("owner #N" when a vault wallet matches), filter (All / Owned by me / With balance), sort (Added / Balance / Label, asc/desc).
- **`src/lib/multisig-utils.ts`**: `WatchlistEntry` gained optional `metadata` and `note` fields (backward-compat with v0 entries). New helpers: `fetchMultisigInfo` (uses `zbx_getMultisig` + `zbx_getBalance`, returns `null` on RPC error code -32004), `updateWatchlistMetadata`, `renameWatchlistEntry`, `parseAddressList`. The `addToWatchlist` helper now folds new metadata + label/note into existing entries when the address is already saved (instead of skipping).
- **In-browser submission of `MultisigOp::Create` is intentionally not implemented** — the chain serializes `Vec<Address>` via serde-as-hex-string in bincode (50 bytes per address: 8-byte length + 42-byte UTF-8) and `MultisigOp` carries nested enum discriminants, so a JS port would be consensus-critical and silent-failure-prone. The CLI command remains the truth source; the UI orchestrates discovery + confirmation + bookmarking around it.
- **DB safety**: trivially n/a — multisig wallets live in on-chain RocksDB, watchlist + history live in browser `localStorage`, vault keys live in browser AES-GCM. No PostgreSQL / Drizzle / schema files were touched.
