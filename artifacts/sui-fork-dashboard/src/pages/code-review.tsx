...
const MOVE_MODULES: ChangeItem[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "NODE_BOND_MIST = 100,000,000,000 (100 ZBX — mandatory node collateral)",
      "  → node bond is SEPARATE from stake — locked in ValidatorStake.node_bond",
      "  → bond returned intact on unstake | future: slash-able for misbehavior",
      ...
      "fn stake(pool, zbx_coin, bond_coin, node_wallet, ctx) — become validator",
      "  → zbx_coin: 10K–250K ZBX (own stake) + bond_coin: exactly 100 ZBX (node bond)",
      "  → E_BOND_WRONG_AMOUNT if bond_coin ≠ exactly 100 ZBX",
      "fn unstake() → (Coin<ZBX>, Coin<ZBX>) — returns (stake + 100 ZBX bond) separately",
      ...
      "fn distribute_epoch_reward() — splits reward between active validators + founder",
      "fn fund_rewards() — direct top-up from genesis treasury",
      "fn node_bond_amount() — view: check locked bond amount on a ValidatorStake",
      "fn global_cap_remaining() — view: how much ZBX still fits in 5M pool",
      "fn is_global_cap_reached() — bool check",
      "E_BOND_WRONG_AMOUNT = 10 — bond must be exactly 100 ZBX",
    ],
  },
  {
