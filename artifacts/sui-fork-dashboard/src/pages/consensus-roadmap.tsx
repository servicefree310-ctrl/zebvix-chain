...
const PHASES: Phase[] = [
  ...
  },
  {
    ...
    tps: "Same (~200)",
    finality: "5 sec",
    validators: "1 producer + N listeners",
    complexity: 2,
    description:
      "Multi-node networking layer. 1 PoA producer + multiple read-only follower nodes. Foundation for everything ahead. No consensus changes yet — just gossip propagation.",
    benefits: [
    ...
    subtasks: [
      { name: "libp2p crate integration", desc: "Add libp2p + tokio dependencies", effort: "0.5 din" },
      { name: "Peer discovery", desc: "mDNS for LAN, Kademlia DHT for WAN", effort: "1 din" },
      { name: "Gossipsub for tx/block broadcast", desc: "Pub-sub topics: 'txs', 'blocks'", effort: "1 din" },
      { name: "Block sync protocol", desc: "Catch-up from peers on startup", effort: "0.5 din" },
      { name: "Chain-id + version handshake", desc: "Reject incompatible peers", effort: "0.5 din" },
      { name: "CLI: --peers flag + bootstrap nodes", desc: "Multi-node spawn", effort: "0.5 din" },
    ],
    files: ["src/p2p.rs (new)", "src/main.rs", "Cargo.toml"],
  },
  {
    id: "phase-b",
    level: 3,
    name: "Phase B — Validator Set & Staking",
    algorithm: "On-Chain Registry",
    icon: Users,
    status: "future",
    duration: "2-3 din",
    tps: "Same",
    finality: "Same",
    validators: "Up to 100 registered",
    complexity: 2,
    description:
      "On-chain validator registry. Anyone can register by staking ZBX. Top 25 by stake become active validator set. Foundation for BFT voting.",
    benefits: [
      "Permissionless validator entry",
      "Economic security via stake",
      "Dynamic validator set rotation",
    ],
    challenges: [
      "Stake unbonding period",
      "Validator set update timing (epoch boundary)",
      "Slashing condition design",
    ],
    subtasks: [
      { name: "register-validator tx type", desc: "New tx variant with pubkey + commission", effort: "0.5 din" },
      { name: "stake / unstake / delegate txs", desc: "Token-locking mechanics", effort: "1 din" },
      { name: "Active set selection", desc: "Top-N by total stake at epoch boundary", effort: "0.5 din" },
      { name: "Validator metadata RPC", desc: "zbx_getValidators endpoint", effort: "0.5 din" },
      { name: "CLI: zbx validator register/stake", desc: "User-facing commands", effort: "0.5 din" },
    ],
    files: ["src/staking.rs (new)", "src/types.rs", "src/state.rs"],
  },
  {
    id: "phase-c",
    level: 4,
    name: "Phase C — Tendermint BFT Consensus",
    ...
    complexity: 3,
    description:
      "Classic Byzantine Fault Tolerance. 3-phase voting (Propose → Pre-vote → Pre-commit). Tolerates up to 8/25 malicious nodes. Instant finality. Foundation chain.",
    benefits: [
