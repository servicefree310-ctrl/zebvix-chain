...
const CHANGES_LOG = [
  ...
  },
  {
    ...
    entries: [
      ...
      { type: "add", text: "MIN_VALIDATOR_STAKE: u64 = 10_000 ZBX — koi bhi is amount ko stake + node run kar ke validator ban sakta hai" },
      { type: "add", text: "VALIDATOR_STAKING_APR: u64 = 120 — validators ko 120% APR milega unke staked amount pe" },
      { type: "add", text: "NODE_DAILY_REWARD: u64 = 5 ZBX/day — node run karne ke liye alag se 5 ZBX daily reward per node" },
      { type: "add", text: "stake(pool, zbx_coin, ctx): koi bhi 10,000 ZBX stake karo + node chalao → validator ban jao" },
      { type: "add", text: "unstake(pool, stake_obj, ctx): validator bahar ja sakta hai — minimum lock period ke baad" },
      { type: "add", text: "claim_rewards(pool, stake_obj, ctx): 120% APR reward claim karo — epoch ke baad" },
      { type: "add", text: "claim_node_reward(pool, node_wallet, ctx): 5 ZBX/day per node — daily claim" },
      { type: "add", text: "Pre-validator period rule: jab tak koi validator nahi → saari staking rewards founder treasury mein jaati hain" },
      { type: "add", text: "Reward split: Validator APR reward validator ko, remaining surplus → founder treasury" },
      { type: "add", text: "NodeWallet: har node runner ke liye dedicated on-chain wallet object (node identity)" },
      { type: "change", text: "Permissionless validator set — whitelist/permission nahi chahiye, sirf stake + node = validator" },
    ],
  },
  {
    phase: "P5",
