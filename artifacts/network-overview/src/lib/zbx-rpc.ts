...
export function weiHexToZbxNum(hex: string | number | bigint): number {
  let n: bigint;
  try {
    n = typeof hex === "bigint" ? hex : BigInt(hex);
  } catch {
...
export function weiToUsd(wei: string | number | bigint, pricePerZbx: number): number {
  return weiHexToZbxNum(wei) * pricePerZbx;
}
...
export function fmtZbx(n: number): string {
