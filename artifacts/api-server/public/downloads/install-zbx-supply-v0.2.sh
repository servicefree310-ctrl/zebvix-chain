#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
#  Zebvix Chain — Supply RPC Patch Installer  (v0.2 — pool seed fix)
# ─────────────────────────────────────────────────────────────────────────────
#  Adds pool_seed_wei + pool_reserve_wei, fixes circulating to include 10M AMM pool seed.
#  Run on each Zebvix VPS node:
#
#    curl -fsSL https://7f6c353a-ec2a-4fe7-81e1-631c9fb77a3e-00-1a0ca41r86kcx.worf.replit.dev/api/downloads/install-zbx-supply-v0.2.sh | sudo bash
#
...
TARBALL_URL="$BASE_URL/zbx-supply-fix-v0.2.tar.gz"
...
mkdir -p "$WORKDIR"
cd "$WORKDIR"
curl -fsSL -o zbx-supply-fix-v0.2.tar.gz "$TARBALL_URL"

# Sanity: must be gzip, not HTML
if ! file zbx-supply-fix-v0.2.tar.gz | grep -q gzip; then
  red "ERROR: download is not a gzip archive. Server may have returned HTML."
  echo "First bytes:"
  head -c 200 zbx-supply-fix-v0.2.tar.gz; echo
  exit 1
fi

cyan "==> 2/6  Extracting"
tar -xzf zbx-supply-fix-v0.2.tar.gz
cd zbx-supply-fix
